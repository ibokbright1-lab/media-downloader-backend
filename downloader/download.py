...
def start_download_task(self, task_id, url, quality="720p", is_audio=False, audio_bitrate="128k"):

    set_task_state(task_id, {"paused": False, "paused_at": None})

    ensure_fresh_or_restart(task_id)

    format_string = build_format_string(quality, is_audio)
...
# ----------------------------
# VIDEO SCALING (CREATE MISSING QUALITY)
# ----------------------------
    ...
    if target_height:
        ...
        try:
            ...
            scaled_path = final_path.replace(".mp4", f"_{quality}.mp4")
